"""
Sequential inference engine for robot manipulation (local, no-Ray runtime).

Mirror of `sequential.sequential_actor.SequentialActor` with the
`@ray.remote` decorator and ray import removed. The orchestration logic
is byte-identical: Controller -> DataManager -> Policy -> DataManager -> Controller.
"""
...
class SequentialLocalActor:
    ...
    def __init__(
        ...
        from env_actor.auto.inference_algorithms.sequential.data_manager.data_manager_interface import DataManagerInterface
    ...
    def start(self) -> None:
        ...
        print(f"obs_data: {obs_data['proprio'].dtype}")
        print(f"obs_data: {obs_data['head'].shape}")
        print(f"obs_data: {obs_data['left'].shape}")
        print(f"obs_data: {obs_data['right'].shape}")

        import cv2
        cv2.imwrite("left.png", np.transpose(obs_data['left'], (1, 2, 0)))
        cv2.imwrite("right.png", np.transpose(obs_data['right'], (1, 2, 0)))
        cv2.imwrite("head.png", np.transpose(obs_data['head'], (1, 2, 0)))
        """
        ...
        if (t % self.controller_interface.policy_update_period) == 0:
            ...
            denormalized_policy_output = self.policy.predict(obs, self.data_normalization_interface)
