...
This file is the igris_c port of the igris_b shm_manager_bridge: same algorithm,
only the action-init shape differs (action_dim=17 init vector built from
INIT_ACTION_17 vs igris_b's 24-D INIT_JOINT_LIST + INIT_HAND_LIST concat).
"""
from __future__ import annotations

from ctypes import c_bool
from env_actor.auto.inference_algorithms.rtc_local.data_manager.utils.shared_memory_utils import ShmArraySpec, attach_shared_ndarray
...
from env_actor.auto.inference_algorithms.rtc_local.data_manager.utils.max_deque import MaxDeque
    ...
    def bootstrap_obs_history(
        ...
        with self._lock:
            for key in obs_history.keys():
                if key == 'proprio':
                    np.copyto(
                        self._shm_array_dict['proprio'],
                        np.repeat(
                            obs_history['proprio'].reshape(1, -1),
