...
class VFP(nn.Module):
    def __init__(self, components, params, normalization):
        ...
        self.prior = components['vqvae_prior']
        self.proprio_projector = components['proprio_projector']
        self.action_decoder = components['moe_action_decoder']
    ...
    def body(self, expert_id: int | None=None):
        ...
        return self.action_decoder.experts[expert_id]
...
class VFPBuilder(PolicyBuilder):
    def build_policy(self, components, params, device=None):
        ...
        policy = VFP(components, params, normalization)
        if device is not None:
            policy.to(device)
        return policy

POLICY_REGISTRY.add("vfp", VFPBuilder())